...
function TeacherMarket() {
    ...
    const [teacherBalance, setTeacherBalance] = useState(user.balance || 0);
    ...
    const loadData = useCallback(async () => {
        ...
        try {
            ...
            if (uData?.id) {
                ...
                sessionStorage.setItem('user', JSON.stringify({ ...currentUser, ...uData }));
            }
    ...
    return (
        <div className="page">
            ...
            <div className="tm-page-head">
                ...
                <div className="tm-wallet-card" style={{maxWidth:'420px',padding:'14px 20px'}}>
                    <div className="tm-wallet-left">
                        ...
                        <div>
                            <div className="tm-wallet-label">Дансны үлдэгдэл</div>
                            <div className="tm-wallet-balance" style={{color:'#059669'}}>{fmtMoney(teacherBalance)}</div>
                        </div>
                    </div>
                ...
                {filteredMarket.length === 0 ? (
                    ...
                    <div key={t.id} className="tm-task-card market-card open" style={{borderLeft:'4px solid #3b82f6'}}>
                        ...
                        {t.attachments?.length > 0 && (
                            <div className="tm-attachment-list" style={{marginTop:'8px'}}>
                                {t.attachments.map((file, i) => (
                                    <a className="tm-attachment-item" href={file.dataUrl} download={file.name} target="_blank" rel="noreferrer" key={file.name + '-' + i}>
                                        <i className={'fas ' + (file.type?.startsWith('image/') ? 'fa-file-image' : file.type === 'application/pdf' ? 'fa-file-pdf' : 'fa-file-alt')}></i>
                                        <span>{file.name}</span>
                                    </a>
                                ))}
                            </div>
                        ...
                        <div className="tm-task-footer">
                            ...
                            <button className="tm-claim-btn" onClick={()=>{setAnswerModal(t);setAnswerText('');setAnswerFiles([]);}}>
            ...
            {answerModal && (
                <div className="tm-overlay open" onClick={e=>{if(e.target===e.currentTarget){setAnswerModal(null);setAnswerText('');setAnswerFiles([]);}}}>
                    <div className="tm-modal tm-modal-lg">
                        <div className="tm-modal-head">
                            <h3><i className="fas fa-pen" style={{color:'#07437d',marginRight:'8px'}}></i>Хариулт бичих</h3>
                            <button className="tm-modal-close" onClick={()=>{setAnswerModal(null);setAnswerText('');setAnswerFiles([]);}}><i className="fas fa-times"></i></button>
                        </div>
                        <div className="tm-modal-body">
                            <div style={{background:'#f8fafc',border:'1px solid #e2e8f0',borderRadius:'12px',padding:'14px 16px',marginBottom:'16px'}}>
                                ...
                                {answerModal.attachments?.length > 0 && (
                                    <div className="tm-attachment-list" style={{marginTop:'10px'}}>
                                        {answerModal.attachments.map((file, i) => (
                                            <a className="tm-attachment-item" href={file.dataUrl} download={file.name} target="_blank" rel="noreferrer" key={file.name + '-' + i}>
                                                <i className={'fas ' + (file.type?.startsWith('image/') ? 'fa-file-image' : file.type === 'application/pdf' ? 'fa-file-pdf' : 'fa-file-alt')}></i>
                                                <span>{file.name}</span>
                                            </a>
                            ...
                            <div className="tm-field">
                                ...
                                <input type="file" multiple accept="image/*,.pdf,.doc,.docx" onChange={e=>setAnswerFiles(Array.from(e.target.files || []))}/>
                                {answerFiles.length > 0 && (
                                    <div className="tm-attachment-list">
                                        {answerFiles.map((f, i) => (
                                            <div className="tm-attachment-item" key={f.name + '-' + i}>
                                                <i className={'fas ' + (f.type.startsWith('image/') ? 'fa-file-image' : f.type === 'application/pdf' ? 'fa-file-pdf' : 'fa-file-alt')}></i>
                                                <span>{f.name}</span>
                                            </div>
                                        ))}
                                    </div>
                        ...
                        <div className="tm-modal-foot">
                            <button className="tm-btn-cancel" onClick={()=>{setAnswerModal(null);setAnswerText('');setAnswerFiles([]);}}>Болих</button>
