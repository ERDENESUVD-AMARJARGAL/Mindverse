                ...
                <div className="tm-overlay open" onClick={e=>{if(e.target===e.currentTarget){setPost(false);setPostError('');}}}>
                    <div className="tm-modal tm-modal-lg">
                        <div className="tm-modal-head">
                            ...
                            <button className="tm-modal-close" onClick={()=>{setPost(false);setPostError('');setPostFiles([]);}}><i className="fas fa-times"></i></button>
                        </div>
                        <form onSubmit={handlePost}>
                            <div className="tm-modal-body">
                                ...
                                <div className="tm-field">
                                    ...
                                    <input type="file" multiple accept="image/*,.pdf,.doc,.docx,.xls,.xlsx" onChange={e=>setPostFiles(Array.from(e.target.files || []))}/>
                                    {postFiles.length > 0 && (
                    ...
                    <div className="tm-modal tm-modal-lg">
                        ...
                        <div className="tm-modal-body">
                            <div style={{background:'#f8fafc',border:'1px solid #e2e8f0',borderRadius:'12px',padding:'16px',marginBottom:'20px'}}>
                                ...
                                {reviewTask.attachments?.length > 0 && (
                                    <div className="tm-attachment-list" style={{marginTop:'10px'}}>
                                        {reviewTask.attachments.map((file, i) => (
                                            <a className="tm-attachment-item" href={file.dataUrl} download={file.name} target="_blank" rel="noreferrer" key={file.name + '-' + i}>
                                                <i className={'fas ' + (file.type?.startsWith('image/') ? 'fa-file-image' : file.type === 'application/pdf' ? 'fa-file-pdf' : 'fa-file-alt')}></i>
                                                <span>{file.name}</span>
                                            </a>
                            ...
                            <div style={{background:'#f0fdf4',border:'1.5px solid #bbf7d0',borderRadius:'12px',padding:'16px',marginBottom:'24px'}}>
                                ...
                                {reviewTask.answerAttachments?.length > 0 && (
                                    <div className="tm-attachment-list" style={{marginTop:'10px'}}>
                                        {reviewTask.answerAttachments.map((file, i) => (
                                            <a className="tm-attachment-item" href={file.dataUrl} download={file.name} target="_blank" rel="noreferrer" key={file.name + '-' + i}>
                                                <i className={'fas ' + (file.type?.startsWith('image/') ? 'fa-file-image' : file.type === 'application/pdf' ? 'fa-file-pdf' : 'fa-file-alt')}></i>
                                                <span>{file.name}</span>
                                            </a>
