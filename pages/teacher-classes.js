                    ...
                    {tab === 'link' ? (
                        <div className="form-group" style={{marginTop:10}}>
                            ...
                            <div className="link-hint">
                                <i className="fas fa-info-circle"/> Сурагчид харах боломжтой линк оруулна уу
                            </div>
                        </div>
                    ) : (
...
function AddClassModal({ open, onClose, onAdd }) {
    ...
    const close = () => {
        setStep('form');
        setForm({ name:'', subject:'Математик', color:COLORS[0], icon:'fa-calculator', maxStudents:30 });
    ...
    const submit = () => {
        ...
        setCode(newCode);
        onAdd({ id:uid('cls'), ...form, name:form.name.trim(), students:0, avg:'-', avgClass:'mid', avgNum:0,
            classCode:newCode, lessons:[], exams:[], assignments:[], studentList:[] });
        setStep('code');
    };
                    ...
                    <>
                        ...
                        <div className="modal-body">
                            ...
                            <div className="info-box blue">
                                <i className="fas fa-info-circle"/> Анги үүсгэхэд автоматаар <strong>нэгдэх код</strong> үүснэ. Сурагчид тэрхүү кодоор нэгдэнэ.
                            </div>
                        </div>
                        <div className="modal-footer">
        ...
        <div className="modal-overlay show" onClick={e=>e.target===e.currentTarget&&onClose()}>
            <div className="modal modal-sm">
                ...
                <div className="modal-body">
                    <div className="form-group">
                        <label>Сурагчийн хязгаар</label>
                        <input type="number" min="1" max="100" value={maxStudents}
...
function LessonPane({ cls, idx, onUpdate, showToast }) {
    ...
    const handleSave = (data) => {
        const updated = editing
            ? lessons.map(l=>l.id===editing.id?{...l,...data}:l)
            : [...lessons, data];
                        ...
                        <div className="lesson-item" key={lsn.id||i}>
                            ...
                            <div className="lesson-info">
                                ...
                                <div className="lesson-meta">
                                    <i className="fas fa-calendar-alt" style={{marginRight:4,fontSize:10}}/>
                                    {lsn.date} {lsn.duration && <><i className="fas fa-clock" style={{marginLeft:8,marginRight:4,fontSize:10}}/>{lsn.duration} мин</>}
                                </div>
                            ...
                            <button className="t-btn-icon" title="Засах" onClick={e=>{e.stopPropagation();setEditing(lsn);setModal(true);}}><i className="fas fa-pen"/></button>
                            <button className="t-btn-icon red" title="Устгах" onClick={e=>{e.stopPropagation();setConfirm(lsn);}}><i className="fas fa-trash"/></button>
                        </div>
                    ))}
                </div>
            )}
...
function ExamPane({ cls, idx, onUpdate, showToast }) {
    ...
    const handleSave = (data) => {
        ...
        fetch(`${API}/classes/${cls.id}`,{
            method:'PATCH',headers:{'Content-Type':'application/json'},
            body:JSON.stringify({exams:updated})
        }).catch(()=>{});
                    ...
                    {exams.map((exm,i)=>{
                        ...
                        return (
                            <div className="detail-exam-item" key={exm.id||i}>
                                ...
                                <button className="t-btn-icon" title="Засах" onClick={e=>{e.stopPropagation();setEditing(exm);setModal(true);}}><i className="fas fa-pen"/></button>
                                <button className="t-btn-icon red" title="Устгах" onClick={e=>{e.stopPropagation();setConfirm(exm);}}><i className="fas fa-trash"/></button>
                            </div>
                        );
                    })}
                </div>
            ...
            {assignments.length === 0 ? (
                ...
                <div className="detail-assign-list">
                    {assignments.map((asn,i)=>{
                        ...
                        const scored    = subs.filter(s=>s.score!=null);
                        const submitted = subs.length || asn.submitted || 0;
                        const total     = asn.total || safe(cls.studentList).length || 1;
                        ...
                        return (
                            <div className="detail-assign-item" key={asn.id||i}>
                                ...
                                <button className="t-btn-icon" title="Засах" onClick={e=>{e.stopPropagation();setEditing(asn);setModal(true);}}><i className="fas fa-pen"/></button>
                                <button className="t-btn-icon red" title="Устгах" onClick={e=>{e.stopPropagation();setConfirm(asn);}}><i className="fas fa-trash"/></button>
                            </div>
                        );
                    })}
                </div>
...
function StudentPane({ cls, idx, onUpdate, showToast }) {
    ...
    const filtered    = studentList.filter(s=>stuName(s).toLowerCase().includes(q.toLowerCase()));
    const sorted      = [...filtered].sort((a,b)=>(b.score||0)-(a.score||0));

    const rankCls    = i => i===0?'gold':i===1?'silver':i===2?'bronze':'';
    const scoreColor = s => s>=75?'#10b981':s>=60?'#f59e0b':'#ef4444';

    const handleRemove = (stu) => {
        const updated = studentList.filter(s=>s.id!==stu.id);
        fetch(`${API}/classes/${cls.id}`,{
            method:'PATCH',headers:{'Content-Type':'application/json'},
            body:JSON.stringify({studentList:updated, students:updated.length})
        }).catch(()=>{});
        fetch(`${API}/users/${stu.id}`,{
            method:'PATCH',headers:{'Content-Type':'application/json'},
            body:JSON.stringify({classId:null,classCode:null})
        }).catch(()=>{});
    ...
    return (
        <div>
            ...
            <div className="info-box blue" style={{marginBottom:16}}>
                <i className="fas fa-info-circle"/> Сурагчид <strong>ангийн кодыг</strong> оруулж нэгдэнэ. Та зөвхөн сурагч <strong>хасах</strong> боломжтой.
            </div>
                            ...
                            <div className="student-info">
                                ...
                                <div className="student-score-row">
                                    <div className="student-score-bar">
                                        <div className="student-score-fill" style={{width:`${stu.score||0}%`,background:scoreColor(stu.score||0)}}/>
                                    </div>
                                    <span className="student-score-pct" style={{color:scoreColor(stu.score||0)}}>{stu.score||0}%</span>
                                </div>
                            </div>
                            <div className={`student-rank ${rankCls(i)}`}>{i+1}</div>
                            <button className="t-btn-icon red stu-remove" title="Ангиас хасах" onClick={()=>setConfirm(stu)}>
        ...
        <div className="class-detail">
            <div className="cd-banner" style={{background:bannerBg}}>
                <div className="cd-banner-top">
                    <div className="cd-banner-icon" style={{background:`color-mix(in srgb, ${cls.color} 14%, #fff)`,color:cls.color,boxShadow:`0 4px 12px color-mix(in srgb, ${cls.color} 25%, transparent)`}}>
                ...
                <div className="cd-kpis">
                    ...
                    <div className="cd-kpi" style={{borderColor:cls.avgNum>=75?'#bbf7d0':cls.avgNum>=60?'#fde68a':'#fecaca'}}>
                        <div className="cd-kpi-val" style={{color:cls.color}}>{cls.avg}</div><div className="cd-kpi-lbl">Ангийн дундаж</div>
                    </div>
                </div>
            </div>
...
function TeacherClasses() {
    ...
    const fetchClasses = useCallback(async () => {
        const teacherId = sessionStorage.getItem('user_id') || sessionStorage.getItem('userId') || 'teacher_1';
        try {
    ...
    const handleAdd = async (newClass) => {
        ...
        try {
            const res = await fetch(`${API}/classes`, {
                ...
                body: JSON.stringify({ ...newClass, teacherId })
            });
    ...
    if (loading) return (
        <div className="page-content" style={{padding:60,textAlign:'center',color:'#64748b'}}>
            <i className="fas fa-spinner fa-spin" style={{fontSize:28,marginBottom:12,display:'block'}}/>Ачаалж байна...
        </div>
    );
    if (error) return (
        <div className="page-content" style={{padding:60,textAlign:'center',color:'#ef4444'}}>
            <i className="fas fa-exclamation-triangle" style={{fontSize:28,marginBottom:12,display:'block'}}/>
            {error}<br/>
            <button className="t-btn primary" style={{marginTop:16}} onClick={fetchClasses}>Дахин оролдох</button>
        </div>
    );
    ...
    return (
        <div className="page-content">
                ...
                {filtered.map(c=>{
                    ...
                    return (
                        <div key={c.id}
                            ...
                            <button className="class-card-delete" onClick={e=>{e.stopPropagation();setDelCls(c);}} title="Анги устгах">
                            ...
                            <div className="class-card-body">
                                ...
                                <div className="cc-capacity">
                                    <div className="cc-cap-header">
                                        <span style={{fontSize:11,color:'#64748b'}}>Хүчин чадал</span>
            ...
            <ConfirmModal open={!!delCls} onClose={()=>setDelCls(null)} onConfirm={()=>handleDeleteClass(delCls)}
                title="Анги устгах" msg={`"${delCls?.name}" ангийг бүрмөсөн устгах уу? Бүх хичээл, шалгалт, даалгавар устгагдана.`} danger/>
        </div>
    );
}
