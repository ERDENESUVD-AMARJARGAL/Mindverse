...
function SignupPage() {
    ...
    const handleAuth = async (e) => {
        ...
        if (view === 'login') {
            try {
                const response = await fetch(`${window.API_URL}/users?email=${formData.email}`);
            ...
            try {
                // Давхар имэйл шалгах
                const checkRes = await fetch(`${window.API_URL}/users?email=${formData.email}`);
                const existing = await checkRes.json();
                if (existing.length > 0) {
                    setErrorMessage("Энэ имэйлээр бүртгэл аль хэдийн байна!");
                    setIsLoading(false);
                    return;
                }

                const newId = role === 'teacher' 
                    ? `teacher_${Date.now()}` 
                    : `stu_${Date.now()}`;
                ...
                if (response.ok) {
                    ...
                    setErrorMessage("Бүртгэл хийхэд алдаа гарлаа.");
                }
            } catch (err) {
                setErrorMessage("Серверт хадгалахад алдаа гарлаа.");
            }
        }
        setIsLoading(false);
    };
            ...
            <div className="right">
                <div className="login-box">
                    ...
                    <form onSubmit={handleAuth}>
                        ...
                        <button type="submit" className={`submit-btn ${role === 'teacher' ? 'teacher-mode' : ''}`} disabled={isLoading}>
                            <i className={view === 'login' ? "fa-solid fa-right-to-bracket" : "fa-solid fa-user-plus"}></i>
                            {isLoading ? 'Түр хүлээнэ үү...' : (view === 'login' ? 'Нэвтрэх' : `${role === 'student' ? 'Сурагчаар' : 'Багшаар'} бүртгүүлэх`)}
                        </button>
                    </form>
